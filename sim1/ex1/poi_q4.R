...
#############################################################################################
# This script corresponds to Example 1 in the main text.
...
# Replace with the path to the folder on your own computer
base_dir <- "~/glmcv/simuV2/result_poi/V2q4"
...
cat("n = ",n_values," p = ",p_values, " q = ",q, " repeat = ",n_simulations,"\n")


cat("Starting simulation with q =", q, "\n")
cat("Testing", length(n_values) * length(p_values), "(n,p) combinations\n")
  ...
  for (p in p_values) {
    ...
    for (s in 1:n_simulations) {
      ...
      re_all <- tryCatch({
        ...
        # Reverse CV with K = 5, 10 (train on single fold, test on rest)
        for (K in c(5, 10)) {
          split_matrices <- split_matrix(n, p, K)
          ...
          # Perform reverse K-fold cross-validation
          for (k in 1:K) {
            # Create train/test masks (reversed)
            train_mask <- split_matrices[[k]]
            test_mask <- Reduce("+", split_matrices[-k]) > 0
          ...
          # ES methods
          mat_es[r, 1] <- which.min(es_loss)
          mat_es[r, 2] <- which.min(es_loss + (K - 1) * (1:8) * pen1)
          mat_es[r, 3] <- which.min(es_loss + (K - 1) * (1:8) * pen2)
          mat_es[r, 4] <- which.min(es_loss + K * (1:8) * pen1)
          mat_es[r, 5] <- which.min(es_loss + K * (1:8) * pen2)
          ...
          mat_es[r, 7] <- which.min(cv_loss + (K * (K - 1)) * (1:8) * pen1)
          mat_es[r, 8] <- which.min(cv_loss + (K * (K - 1)) * (1:8) * pen2)
          mat_es[r, 9] <- which.min(cv_loss + (K * K) * (1:8) * pen1)
          mat_es[r, 10] <- which.min(cv_loss + (K * K) * (1:8) * pen2)
        }
        ...
        c(
          as.vector(t(mat_es)),  # CV results 
          re_jic1, re_jic2,     # JIC results 
          re_aic1, re_aic2,     # AIC results 
          re_bic1, re_bic2, re_bic3,  # BIC results 
          re_bcv1, re_bcv2,     # BCV results
          re_ladle,             
          re_ratio,             
          re_samplecv2,         # Sample CV2 result
          re_samplecv_bayes     # Sample CV Bayes result 
        )
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-cada.rds", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-cada.rds", base_dir, n, p, q)
    ...
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-cada.xlsx", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-cada.xlsx", base_dir, n, p, q)
