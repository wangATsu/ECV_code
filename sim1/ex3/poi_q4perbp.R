#############################################################################################
# This script corresponds to Example 3 in the main text.
...
base_dir <- "~/glmcv/simuV2/result_poi/V2q4perbp"
  ...
  for (p in p_values) {
    ...
    for (s in 1:n_simulations) {
      ...
      re_all <- tryCatch({
        ...
        ##### perb-p
        start_row <- floor(0.9 * p) + 1  # Calculate the starting column for the last 10%
        resp[,start_row:p] <- resp[,start_row:p] * 2
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-perbp-cada.rds", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-perbp-cada.rds", base_dir, n, p, q)
    ...
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-perbp-cada.xlsx", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-perbp-cada.xlsx", base_dir, n, p, q)
