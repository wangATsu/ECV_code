#############################################################################################
# This script corresponds to Example 4 in the main text.
...
base_dir <- "~/glmcv/simuV2/result_poi/V2q4weak"
...
sim_weak <- function(N, J, K){
  F.matr <- cbind(matrix(runif(N*(K-1),-2,2), N, (K-1)),matrix(runif(N*(1),-0.3,0.3), N, (1)));
...
for (n in n_values) {
  for (p in p_values) {
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-weak-03-cada.rds", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-weak-03-cada.rds", base_dir, n, p, q)
    ...
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-weak-03-cada.xlsx", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-weak-03-cada.xlsx", base_dir, n, p, q)
