#############################################################################################
# This script corresponds to Example 6 in the main text.
...
base_dir <- "~/glmcv/simuV2/result_poi/V2overdis"
  ...
  for (p in p_values) {
    ...
    for (s in 1:n_simulations) {
      ...
      re_all <- tryCatch({
        ...
        # ECV with K = 5 only (K = 2 and K = 10 commented out)
        for (K in c(5)) { 
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-overdis1-10-cada.rds", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-overdis1-10-cada.rds", base_dir, n, p, q)
    
    saveRDS(results, file = filename_facnum)
    saveRDS(re_frequ, file = filename_frequency)
    
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-overdis1-10-cada.xlsx", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-overdis1-10-cada.xlsx", base_dir, n, p, q)
