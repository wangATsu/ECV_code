#############################################################################################
# This script corresponds to reproduce Ex 2 in Table 1 in the main text.
...
# -----------------------------------------------------------------------------
base_folder <- "~/glmcv/simuV2/result_poi/V2q4_othernp/"  
output_folder <- "~/glmcv/simuV2/result_poi/V2q4_othernp/" 
file_prefix <- "facnum-cada"  


n_values <- c(30,50, 100, 300,500,1000)  
p_values <- c(30,50, 100, 300,500,1000)  
q_value <- 4  
...
process_single_file <- function(filepath, n_val, p_val, q_val, target_cols) {
  ...
  if(is.data.frame(data)) {
   ...
   # cat("unknown class type", class(data), "\n")
    data <- as.matrix(data)
  }
...
rds_filename <- file.path(output_folder, "result_facnum_mat.rds")
saveRDS(result_facnum_mat, rds_filename)
cat("RDS save as ", rds_filename, "\n")


library(openxlsx)
xlsx_filename <- file.path(output_folder, "result_facnum_mat.xlsx")
