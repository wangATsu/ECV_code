#############################################################################################
# This script corresponds to Example 5 in the main text.
...
base_dir <- "~/glmcv/simuV2/result_poi/V2q4_miss"
  ...
  for (p in p_values) {
    ...
    for (s in 1:n_simulations) {
      ...
      re_all <- tryCatch({
        ...
        resp <- data$resp
        ## missing setting
        miss <- matrix(rbinom(n * p, size = 1, prob = prob), nrow = n, ncol = p)
        resp[miss==0] = NA
        
        
        resp_com=resp
        for (i in 1:ncol(resp_com)) {
          col_mean <- floor(mean(resp_com[,i], na.rm = TRUE)) # 
          resp_com[ is.na(resp[,i]),i] <- col_mean      #
        }

        
        C_ada=max(estimate_C(log(resp_com+1),qmax=9)$C_est,5)
        ...
        for (K in c(5)) {
          ...
          for (k in 1:K) {
            ...
            test_mask=test_mask*miss==1
            train_mask=train_mask*miss == 1
        ...
        exist_obs=sum(miss)
        resp_zero=resp
        resp_zero[is.na(resp_zero)] = 0
        
        tol_full <- 0.01 / (exist_obs)
        ...
        for (q_est in 1:8) {
          jic_results[[q_est]] <- confirm_CJMLE_poisson_cpp(
            resp_zero, exist_matrix, 
            theta0[, 1:(q_est + 1)], A0[, 1:(q_est + 1)], 
            matrix(TRUE, p, q_est + 1), 
            C = C_ada, tol = tol_full
          )
          
          jic_result <- JL(resp_zero, jic_results[[q_est]]$theta, jic_results[[q_est]]$A, exist_matrix)
        ...
        for (i in 1:ncol(resp)) {
          col_mean <- floor(mean(resp[,i], na.rm = TRUE)) # 
          resp[ is.na(resp[,i]),i] <- col_mean      #
        }
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-miss-cada.rds", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-miss-cada.rds", base_dir, n, p, q)
    ...
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-miss-cada.xlsx", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-miss-cada.xlsx", base_dir, n, p, q)
