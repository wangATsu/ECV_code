#############################################################################################
# This script corresponds to Example S3 in the supplementary materials.
...
base_dir <- "~/glmcv/simuV2/result_norm/V2norm"
...
Rcpp::sourceCpp("~/glmcv/simuV2/confirm_jmle_omp_linear_with_intercept_missing.cpp")
  ...
  for (p in p_values) {
    ...
    for (s in 1:n_simulations) {
      ...
      re_all <- tryCatch({
        ...
        data <- sim1_norm(n, p, q)
        resp <- data$resp
        
        estc=estimate_C(resp,qmax = 9)$C_est
        C_ada=max(estc,9)
          ...
          for (k in 1:K) {
            ...
            for (q_est in 1:8) {
              jml_res <- CJMLE_linear(
                ...
                C = C_ada, tol = tol_val, F
              )
              
              jml_result <- JL_norm(resp, jml_res$theta, jml_res$A, test_mask)
        ...
        for (q_est in 1:8) {
          jic_results[[q_est]] <- CJMLE_linear(
            resp, true_matrix, 
            theta0[, 1:(q_est + 1)], A0[, 1:(q_est + 1)], 
            matrix(TRUE, p, q_est + 1), 1,
            C = C_ada, tol = tol_full,F
          )
          
          jic_result <- JL_norm(resp, jic_results[[q_est]]$theta, jic_results[[q_est]]$A, true_matrix)
       ...
       # ladle <- PCAladle(resp, ncomp = 8)
        ...
        # Sample CV methods
        samplecv2_result <- factor_selection_cv_normal(
          resp = resp,
          theta0 = theta0,
          A0 = A0,
          K = 5,
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-norm-cada.rds", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-norm-cada.rds", base_dir, n, p, q)
    ...
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-norm-cada.xlsx", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-norm-cada.xlsx", base_dir, n, p, q)
