#############################################################################################
# This script corresponds to Example S2 in the supplementary materials.
...
base_dir <- "~/glmcv/simuV2/result_poi/V2q4perbn"
  ...
  for (p in p_values) {
    ...
    for (s in 1:n_simulations) {
      ...
      re_all <- tryCatch({
        ...
        ##### perb-n
        start_row <- floor(0.9 * n) + 1  
        resp[start_row:n,] <- resp[start_row:n,] * 2
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-perbn-cada.rds", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-perbn-cada.rds", base_dir, n, p, q)
    ...
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-perbn-cada.xlsx", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-perbn-cada.xlsx", base_dir, n, p, q)
