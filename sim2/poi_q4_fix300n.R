#############################################################################################
# This script corresponds to Figure S2 in the supplementary materials.
...
n_values <- c(300)
p_values <- c(30,50,75,100,150,200,400,500,600,700,300,800,900,1000)
...
for (n in n_values) {
  for (p in p_values) {
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-fixn-cada.rds", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-fixn-cada.rds", base_dir, n, p, q)
    ...
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-fixn-cada.xlsx", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-fixn-cada.xlsx", base_dir, n, p, q)
