#############################################################################################
# This script corresponds to Figure S2 (left plot) in the supplementary materials.
...
n_values <- c(30,50,75,100,150,200,300,400,500,600,700,800,900,1000)
p_values <- c(300)
  ...
  for (p in p_values) {
    ...
    for (s in 1:n_simulations) {
      ...
      re_all <- tryCatch({
        ...
        re_bcv1 <- NA
        #which.min(as.vector(colMeans(bcv1$msep))[-1])
        ...
        # )
        re_samplecv2 <- NA
        #samplecv2_result$optimal_factors
        ...
        #   seed = s
        # )
        re_samplecv_bayes <- NA
        #cv_results_bayes$facnum_samplecv
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-fixp-cada.rds", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-fixp-cada.rds", base_dir, n, p, q)
    ...
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_facnum-fixp-cada.xlsx", base_dir, n, p, q)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_frequency-fixp-cada.xlsx", base_dir, n, p, q)
