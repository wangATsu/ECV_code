#############################################################################################
# This script corresponds to real data 10 fold ECV with missing .
...
for (n in c(707)) {
  for (p in c(1000)) {
    ...
    est_c=estimate_C(log(resp_com+1),qmax = 9)
    
    init=GFM::gfm(list(resp_com),types = 'poisson',q=8,verbose = F)
