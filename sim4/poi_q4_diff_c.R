#############################################################################################
# Choose the constraint constant C
# This script corresponds to Figure S1 (left plot) in the supplementary materials.
...
base_dir <- "~/glmcv/simuV2/estimate_c"
  ...
  for (p in p_values) {
    ...
    for (s in 1:n_simulations) {
      ...
      re_all <- tryCatch({
        ...
        # ===============================
        # Entrywise splitting Cross-Validation Methods
        # ===============================
    ...
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_C%d_facnum.rds", base_dir, n, p, q, C)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_C%d_frequency.rds", base_dir, n, p, q, C)
    ...
    # Save results for this (n,p) combination
    filename_facnum <- sprintf("%s/n%d_p%d_q%d_C%d_facnum.xlsx", base_dir, n, p, q, C)
    filename_frequency <- sprintf("%s/n%d_p%d_q%d_C%d_frequency.xlsx", base_dir, n, p, q, C)
